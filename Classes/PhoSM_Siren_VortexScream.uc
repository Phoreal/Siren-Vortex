class PhoSM_Siren_VortexScream extends KFSM_GrappleCombined;
...
{
	local KFPawn_ZedSiren SirenOwner;

    Super.SpecialMoveStarted( bForced, PrevMove );

	SirenOwner = KFPawn_ZedSiren( PawnOwner );
    ...
    if ( SirenOwner.Role == ROLE_Authority )
    {
        SirenOwner.SetTimer( VortexCheckTime, true, nameOf(Timer_CheckVortex), self );

        // Calculate our damage
        FollowerDamagePerSec = SirenOwner.GetRallyBoostDamage( int(DamageOverDuration / VortexDuration) );
    }
	...
	if( SirenOwner.IsLocallyControlled() )
	{
		SirenOwner.SetTimer( MinVortexDuration, false, nameOf(Timer_VortexInterrupt), self );
	}

    SirenOwner.BumpFrequency = 0.f;

    // View constraints
    SirenOwner.ViewPitchMin = ViewPitchConstraints.X;
    SirenOwner.ViewPitchMax = ViewPitchConstraints.Y;

    // Head tracking
    if( SirenOwner.IK_Look_Head == none )
	{
		SirenOwner.IK_Look_Head = SkelControlLookAt( SirenOwner.Mesh.FindSkelControl('HeadLook') );
	}
	SirenOwner.bCanHeadTrack = true;
    SirenOwner.bIsHeadTrackingActive = true;
	SirenOwner.MyLookAtInfo.LookAtPct = 1.f;
	SirenOwner.MyLookAtInfo.BlendOut = 0.33f;
	SirenOwner.MyLookAtInfo.BlendIn = 0.2f;

    // Spawn particle effect, play sound
    if( SirenOwner.WorldInfo.NetMode != NM_DedicatedServer )
    {
		VortexPSC = SirenOwner.WorldInfo.MyEmitterPool.SpawnEmitterMeshAttachment( VortexEffect, SirenOwner.Mesh, 'VortexSocket', true );
		VortexPSC.SetAbsolute( false, true );
		VortexPSC.SetRotation( SirenOwner.Rotation );

		// Post ak event on owner
		SirenOwner.PostAkEvent( VortexLoopAkEvent, true, true, true );
	}
}
...
function ProcessViewRotation( float DeltaTime, out rotator out_ViewRotation, out Rotator out_DeltaRot )
{
	if( Follower != none )
	{
		out_ViewRotation = RInterpTo( out_ViewRotation, AIOwner.Rotation, DeltaTime, ViewRotInterpSpeed );
...
function Timer_CheckVortex()
{
	...
	foreach AIOwner.WorldInfo.AllPawns( class'KFPawn', KFP )
	{
		if( KFP.GetTeamNum() != AIOwner.GetTeamNum() && CanInteractWithPawn(KFP) )
		{
			...
			if( DistSQ <= MaxRangeSQ )
			{
				...
				if( FOV > MinGrabTargetFOV )
				{
					...
					if( IsPawnPathClear(AIOwner, KFP, GrabLocation, TraceStart, vect(2,2,2),, true)
						&& IsPawnPathClear(AIOwner, KFP, GrabLocation, TraceStart,,, true) )
					{
	...
	{
		// Set our attach time
		FollowerAttachTime = AIOwner.WorldInfo.TimeSeconds;
...
function StartInteraction() 
{
	if( Follower != none )
	{
		...
		// Play a grab sound
		if( AIOwner.WorldInfo.NetMode != NM_DedicatedServer )
		{
			AIOwner.PostAkEvent( VortexGrabAkEvent, true, true, true );
		}

        ++KFPlayerReplicationInfoVersus(AIOwner.PlayerReplicationInfo).ZedGrabs;
	}
...
function Timer_DamageFollower()
{
	...
	if( AIOwner.WorldInfo.TimeSeconds - FollowerAttachTime >= VortexDuration )
	{
		KFPOwner.EndSpecialMove();
		return;
	}
	...
	{
		GrabDirection = Normal( AIOwner.Location - Follower.Location );
		GrabLocation = Follower.Location + (GrabDirection * Follower.CylinderComponent.CollisionRadius); 
	    Follower.TakeDamage( FollowerDamagePerSec, KFPOwner.Controller, GrabLocation, GrabDirection, VortexDamageType,, AIOwner );
	    ...
	    KFPawn_ZedSiren(PawnOwner).MeleeAttackHelper.PlayMeleeHitEffects( Follower, GrabLocation, GrabDirection, false );
	}
}
...
function SpecialMoveEnded(Name PrevMove, Name NextMove)
{
	...
	if( KFPOwner != none )
	{
	    ...
	    {
		    KFPawn_ZedSiren(PawnOwner).BumpFrequency = KFPawn_ZedSiren(PawnOwner).default.BumpFrequency;
			KFPOwner.ClearTimer( nameOf(Timer_CheckVortex), self );
...
defaultproperties
{
    Handle=PhoSM_Siren_VortexScream
